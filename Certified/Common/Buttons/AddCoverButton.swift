...
class AddCoverButton: UIButton {
    ...
    init(frame: CGRect = .zero, onTap: (() -> Void)?) {
    ...
    private func setUpAttributedTitle() {
        ...
        if let image = UIImage(systemName: "plus.app") {
            ...
            attributedString.append(NSAttributedString(string: " \(LocalizedString.Create.addToCollectionTitle)"))
        } else {
            attributedString = NSMutableAttributedString(string: LocalizedString.Create.addToCollectionTitle)
        }
    ...
    private func setUpColors(for theme: ThemeManager.Theme) {
        backgroundColor = Colors.CreateTab.addCoverButtonBackgroundColor.uiKitColor(for: theme)
        layer.shadowOffset = CGSize(width: 0, height: 2)
        layer.shadowOpacity = 1
        layer.shadowRadius = 8
        layer.shadowColor = Colors.CreateTab.addCoverButtonShadowColor.uiKitColor(for: theme).cgColor
